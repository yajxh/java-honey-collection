 ...
 */
public class SyCache107Manager implements CacheManager {

    private final SyCaching107Provider cachingProvider;
    ...
    private final ConcurrentMap<String, SyCache107<?, ?>> caches = new ConcurrentHashMap<>();

    public SyCache107Manager(SyCaching107Provider cachingProvider, Properties props, ClassLoader classLoader, URI uri) {
    ...
    synchronized public <K, V, C extends Configuration<K, V>> Cache<K, V> createCache(String cacheName, C configuration)
        ...
        SyCache107<?, ?> cache = this.caches.get(cacheName);

        if (cache == null) {
            cache = new SyCache107<>(this, cacheName, configuration);
    ...
    public <K, V> Cache<K, V> getCache(String cacheName, Class<K> keyClazz, Class<V> valueClazz) {
        ...
        SyCache107<K, V> cache = (SyCache107<K, V>) this.caches.get(cacheName);
